...
const borderType: {[key: number]: string} = {
    ...
    1: "border-l-0 border-t-0",
    2: "border-l-0 border-t-0"
}
const Hero = () => {
  return (
    <div className="bg-dark">
      <div className="flex justify-between pt-32 px-16">
      ...
      <div className="flex gap-4 justify-center items-center">
        ...
        <button className="bg-lightGrey flex items-center rounded w-32 px-1">
          ...
          <div className="flex flex-col gap-0.5 py-1 px-0.5">
            <span className="text-[10px]">Get if from</span>
            <span className="text-sm">Google Play</span>
          </div>
        </button>
      </div>

      <div className="flex justify-end items-center gap-8 py-16 mr-52">
        {data.map((item, i) => (
          <span
            key={item.title}
            className="border-2 relative border-white w-24 h-24 rounded-full inline-flex gap-1 items-center flex-col justify-center"
          >
            <span className={`absolute w-24 h-24 rounded-full border-2 ${borderType[i]} border-primary`} />
